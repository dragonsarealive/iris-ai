...
<p align="center"><strong>IRIS</strong> — オープンソースのAIコーディングエージェント（OpenCode のフォーク）。</p>
<p align="center">
...
# パッケージマネージャー
npm i -g iris-code@latest            # bun/pnpm/yarn でもOK
bunx iris-code                      # run without installing
brew install dragonsarealive/tap/iris # macOS と Linux（推奨。常に最新）
```
...
IRIS はデスクトップアプリとしても利用できます。[releases page](https://github.com/dragonsarealive/iris-ai/releases) から直接ダウンロードしてください。
...
1. `$IRIS_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.iris/bin` - デフォルトのフォールバック
...
IRIS は従来の `OPENCODE_*` フラグに対応する **`IRIS_*` ミラー**（例: `IRIS_SERVER_PASSWORD`、`IRIS_EXPERIMENTAL_AGENT_TEAMS`）を受け付けます。`OPENCODE_*` が未設定のときは起動時に `IRIS_*` の値がコピーされます。新しいスクリプトでは `IRIS_*` を推奨します。


### Agents

IRIS には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
IRIS の設定については [upstream OpenCode documentation](https://opencode.ai/docs) を参照してください。IRIS 固有のドキュメントは irislab.dev にあります。

### コントリビュート

IRIS に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。
...
- 100% オープンソース
- 特定のプロバイダーに依存しません。[OpenCode Zen](https://opencode.ai/zen) で提供しているモデルを推奨しますが、IRIS は Claude、OpenAI、Google、またはローカルモデルでも利用できます。モデルが進化すると差は縮まり価格も下がるため、provider-agnostic であることが重要です。
- そのまま使える LSP サポート
- TUI にフォーカス。IRIS は neovim ユーザーと [terminal.shop](https://terminal.shop) の制作者によって作られており、ターミナルで可能なことの限界を押し広げます。
- クライアント/サーバー構成。例えば IRIS をあなたのPCで動かし、モバイルアプリからリモート操作できます。TUI フロントエンドは複数あるクライアントの1つにすぎません。
