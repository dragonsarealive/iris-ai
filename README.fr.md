...
<p align="center"><strong>IRIS</strong> — L'agent de codage IA open source (fork d'OpenCode).</p>
<p align="center">
...
brew install dragonsarealive/tap/iris # macOS et Linux (recommandé, toujours à jour)
```
...
IRIS est aussi disponible en application de bureau. Téléchargez-la directement depuis la [page des releases](https://github.com/dragonsarealive/iris-ai/releases).
...
1. `$IRIS_INSTALL_DIR` - Répertoire d'installation personnalisé
2. `$XDG_BIN_DIR` - Chemin conforme à la spécification XDG Base Directory
3. `$HOME/bin` - Répertoire binaire utilisateur standard (s'il existe ou peut être créé)
4. `$HOME/.iris/bin` - Repli par défaut
...
IRIS accepte des **miroirs `IRIS_*`** des anciens indicateurs `OPENCODE_*` (p. ex. `IRIS_SERVER_PASSWORD`, `IRIS_EXPERIMENTAL_AGENT_TEAMS`). Si la variable `OPENCODE_*` n'est pas définie, la valeur `IRIS_*` est copiée au démarrage. Préférez `IRIS_*` pour les nouveaux scripts.


### Agents

IRIS inclut deux agents intégrés que vous pouvez basculer avec la touche `Tab`.
...
Pour plus d'informations sur la configuration d'IRIS, consultez la [documentation upstream d'OpenCode](https://opencode.ai/docs). La documentation spécifique à IRIS est disponible sur irislab.dev.

### Contribuer

Si vous souhaitez contribuer à IRIS, lisez nos [docs de contribution](./CONTRIBUTING.md) avant de soumettre une pull request.
...
- 100% open source
- Pas couplé à un fournisseur. Nous recommandons les modèles proposés via [OpenCode Zen](https://opencode.ai/zen) ; IRIS peut être utilisé avec Claude, OpenAI, Google ou même des modèles locaux. Au fur et à mesure que les modèles évoluent, les écarts se réduiront et les prix baisseront, donc être agnostique au fournisseur est important.
- Support LSP prêt à l'emploi
- Un focus sur la TUI. IRIS est construit par des utilisateurs de neovim et les créateurs de [terminal.shop](https://terminal.shop) ; nous allons repousser les limites de ce qui est possible dans le terminal.
- Architecture client/serveur. Cela permet par exemple de faire tourner IRIS sur votre ordinateur tout en le pilotant à distance depuis une application mobile. Cela signifie que la TUI n'est qu'un des clients possibles.
