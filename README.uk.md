...
<p align="center"><strong>IRIS</strong> — AI-агент для програмування з відкритим кодом (форк OpenCode).</p>
<p align="center">
...
# Менеджери пакетів
npm i -g iris-code@latest           # або bun/pnpm/yarn
bunx iris-code                      # run without installing
brew install dragonsarealive/tap/iris # macOS і Linux (рекомендовано, завжди актуально)
```
...
IRIS також доступний як десктопний застосунок. Завантажуйте напряму зі [сторінки релізів](https://github.com/dragonsarealive/iris-ai/releases).
...
1. `$IRIS_INSTALL_DIR` - Користувацький каталог встановлення
2. `$XDG_BIN_DIR` - Шлях, сумісний зі специфікацією XDG Base Directory
3. `$HOME/bin` - Стандартний каталог користувацьких бінарників (якщо існує або його можна створити)
4. `$HOME/.iris/bin` - Резервний варіант за замовчуванням
...
IRIS приймає **дзеркала `IRIS_*`** для застарілих прапорів `OPENCODE_*` (наприклад, `IRIS_SERVER_PASSWORD`, `IRIS_EXPERIMENTAL_AGENT_TEAMS`). Якщо `OPENCODE_*` не задано, під час запуску копіюється значення `IRIS_*`. У нових скриптах надавайте перевагу `IRIS_*`.


### Агенти

IRIS містить два вбудовані агенти, між якими можна перемикатися клавішею `Tab`.
...
Щоб дізнатися більше про налаштування IRIS, перейдіть до [документації upstream OpenCode](https://opencode.ai/docs). Документація, специфічна для IRIS, доступна на [irislab.dev](https://irislab.dev).

### Внесок

Якщо ви хочете зробити внесок в IRIS, будь ласка, прочитайте нашу [документацію для контриб'юторів](./CONTRIBUTING.md) перед надсиланням pull request.
...
- 100% open source
- Немає прив'язки до конкретного провайдера. Ми рекомендуємо моделі, які надаємо через [OpenCode Zen](https://opencode.ai/zen), але IRIS також працює з Claude, OpenAI, Google і навіть локальними моделями. З розвитком моделей різниця між ними зменшуватиметься, а ціни падатимуть, тому незалежність від провайдера має значення.
- Підтримка LSP з коробки
- Фокус на TUI. IRIS створено користувачами neovim та авторами [terminal.shop](https://terminal.shop); ми й надалі розширюватимемо межі можливого в терміналі.
- Клієнт-серверна архітектура. Наприклад, це дає змогу запускати IRIS на вашому комп'ютері й керувати ним віддалено з мобільного застосунку, тобто TUI-фронтенд - лише один із можливих клієнтів.
