...
test("custom provider without models in database is still added via config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
          provider: {
            // This provider doesn't exist in the database and has no models defined
            "brand-new-standalone": {
              name: "Brand New Standalone",
              npm: "@ai-sdk/openai-compatible",
              api: "https://api.example.com/v1",
              env: [],
              options: {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const providers = await Provider.list()
      // Provider should be added even without models in config
      expect(providers["brand-new-standalone"]).toBeDefined()
      expect(providers["brand-new-standalone"].name).toBe("Brand New Standalone")
      // models should be empty object (not undefined)
      expect(providers["brand-new-standalone"].models).toBeDefined()
      expect(Object.keys(providers["brand-new-standalone"].models).length).toBe(0)
    },
  })
})
