  ...
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      if (instance.project.vcs === "git") {
        const dotgitPath = path.join(instance.directory, ".git")
        const isLinkedWorktree = existsSync(dotgitPath) && statSync(dotgitPath).isFile()

        if (isLinkedWorktree) {
          if (!cfgIgnores.includes(".git") && !cfgIgnores.includes(dotgitPath)) {
            yield* subscribe(dotgitPath, [])
          }
        } else {
