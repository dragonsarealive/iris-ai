...
export namespace Provider {
  ...
  const state = Instance.state(async () => {
    ...
    // Track which providers are custom (not in modelsDev) - needed for filtering
    ...
    function mergeProvider(providerID: ProviderID, provider: Partial<Info>) {
      ...
      // @ts-expect-error
      providers[providerID] = mergeDeep(match ?? { id: providerID, models: {} }, provider)
    }

    // extend database from config
    for (const [providerID, provider] of configProviders) {
      const existing = database[providerID]
      // Track if this is a custom provider (not in modelsDev)
      if (!existing) {
    ...
    for (const [id, provider] of configProviders) {
      const providerID = ProviderID.make(id)
      const partial: Partial<Info> = { source: "config", models: {} }
    ...
    for (const [id, provider] of Object.entries(providers)) {
      ...
      if (Object.keys(provider.models).length === 0) {
        // Allow custom providers (not in modelsDev) with empty models
        // These providers are valid and can have models added later via UI
