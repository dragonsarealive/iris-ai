...
export namespace Installation {
  ...
  /** Curl installer script URL: still upstream until this fork publishes its own install host (see Brew/Scoop names in uninstall). */
  ...
  async function getBrewFormula() {
    const tapFormula = await text(["brew", "list", "--formula", "dragonsarealive/tap/iris"])
    if (tapFormula.includes("iris")) return "dragonsarealive/tap/iris"
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      ...
      case "brew": {
        ...
        if (formula.includes("/")) {
          const tap = await Process.run(["brew", "tap", "dragonsarealive/tap"], { env, nothrow: true })
          if (tap.code !== 0) {
            result = tap
            break
          }
          const repo = await Process.text(["brew", "--repo", "dragonsarealive/tap"], { env, nothrow: true })
          if (repo.code !== 0) {
            result = repo
            break
          }
  ...
  export const USER_AGENT = `iris/${CHANNEL}/${VERSION}/${Flag.OPENCODE_CLIENT}`
  ...
  export async function latest(installMethod?: Method) {
    ...
    return fetch("https://api.github.com/repos/dragonsarealive/iris-ai/releases/latest")
      .then((res) => {
        if (!res.ok) throw new Error(res.statusText)
        return res.json()
      })
