...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
      for (const file of await ConfigPaths.projectFiles("opencode", Instance.directory, Instance.repoRoot)) {
    ...
    const directories = await ConfigPaths.directories(Instance.directory, Instance.repoRoot)
  ...
  export async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies["@iris-ai/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@iris-ai/plugin", depVersion, dir)
  ...
  export const Info = z
    .object({
      ...
      experimental: z
        .object({
          ...
          continue_loop_on_deny: z
            ...
            .optional()
            .default(true)
            .describe("Continue the agent loop when a tool call is denied (default: true)"),
          mcp_timeout: z
            .number()
            .int()
            .positive()
      ...
      team: z
        .object({
          enabled: z.boolean().optional().describe("Enable agent teams feature"),
          max_teammates: z.number().int().positive().optional().describe("Maximum teammates per team"),
          teammate_mode: z
            .enum(["in-process", "worktree"])
            .optional()
            .describe("How teammate sessions are created"),
        })
        .optional()
        .describe("Agent teams configuration"),
    })
    .strict()
    .meta({
      ref: "Config",
