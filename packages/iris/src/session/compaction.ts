...
---`
  ...
  const DB_RETENTION_MS = DB_RETENTION_DAYS * 24 * 60 * 60 * 1000
  ...
  export async function scheduleCleanup() {
    ...
    setInterval(async () => {
      try {
        ...
        await pruneLargeBashOutputs()
      } catch (err) {
        log.error("scheduled cleanup failed", { err })
      }
    }, CLEANUP_INTERVAL_MS)

    log.info("cleanup scheduler started", { intervalHours: CLEANUP_INTERVAL_MS / (60 * 60 * 1000) })
  }
  ...
  async function pruneLargeBashOutputs() {
    ...
    const largeParts = Database.use((db) =>
      db
        .select({ id: PartTable.id, session_id: PartTable.session_id, data: PartTable.data })
        .from(PartTable)
    ...
    for (const part of largeParts) {
      ...
      if (
        ...
        data.state.output.length > PRUNE_BASH_MAX_BYTES
      ) {
        Database.use((db) => {
          db.update(PartTable)
            .set({
              data: {
                ...
                state: {
                  ...data.state,
                  output: data.state.output.slice(0, PRUNE_BASH_MAX_BYTES) + "\n\n[Output truncated by cleanup]",
                },
              },
            })
    ...
    if (pruned > 0) {
      log.info("pruned large bash outputs", { count: pruned })
    }
  }
  ...
  export async function cleanupOldData() {
    ...
    const cutoff = Date.now() - DB_RETENTION_MS
    log.info("cleaning up old data", { cutoff, retentionDays: DB_RETENTION_DAYS })
    ...
    if (oldSessions.length === 0) {
      log.info("no old sessions to clean up")
      return
    }
    ...
    log.info("cleaned up old sessions", { count: oldSessions.length })
  }
}
