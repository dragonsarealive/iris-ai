...
export namespace MessageV2 {
  ...
  /** Throws if any assistant message has a tool part in pending/running (orphaned tool_use). */
  export function validateToolIntegrity(msgs: WithParts[]) {
    for (const msg of msgs) {
      ...
      for (const part of msg.parts) {
        ...
        if (part.state.status === "pending" || part.state.status === "running") {
          throw new Error(
            `orphaned tool_use: message ${msg.info.id} has tool part ${part.id} (${part.tool}) in ${part.state.status} without tool_result`,
          )
        }
      }
    }
