...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run IRIS",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                UI.println(`Found IRIS session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["iris", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting IRIS...")
        UI.println()

        // Launch IRIS TUI with session ID if available (`opencode` remains an install alias on some systems)
        const { spawn } = await import("child_process")
        const argv = sessionId ? ["-s", sessionId] : []
        const child = spawn("iris", argv, {
          stdio: "inherit",
          cwd: process.cwd(),
        })
        ...
        await new Promise<void>((resolve, reject) => {
          child.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`iris exited with code ${code}`))
          })
