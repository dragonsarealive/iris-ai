...
export namespace LSP {
  ...
  export async function touchFile(input: string, waitForDiagnostics?: boolean) {
    ...
    const notifyPromises = clients.map(async (client) => {
      try {
        ...
        log.error("failed to notify open", { err, file: input })
      }
    })

    if (waitForDiagnostics) {
      const diagnosticPromises = clients.map((client) => client.waitForDiagnostics({ path: input }).catch(() => {}))
      await Promise.race([
        Promise.all([...notifyPromises, ...diagnosticPromises]),
  ...
  export async function touchFileFireForget(input: string) {
    const clients = await getClients(input)
