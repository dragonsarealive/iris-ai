...
<p align="center"><strong>IRIS</strong> — 오픈 소스 AI 코딩 에이전트(OpenCode 포크).</p>
<p align="center">
...
# 패키지 매니저
npm i -g iris-code@latest           # bun/pnpm/yarn 도 가능
bunx iris-code                      # run without installing
brew install dragonsarealive/tap/iris # macOS 및 Linux (권장, 항상 최신)
```
...
IRIS 는 데스크톱 앱으로도 제공됩니다. [releases page](https://github.com/dragonsarealive/iris-ai/releases) 에서 직접 다운로드하세요.
...
IRIS는 기존 `OPENCODE_*` 플래그에 대한 **`IRIS_*` 미러**(예: `IRIS_SERVER_PASSWORD`, `IRIS_EXPERIMENTAL_AGENT_TEAMS`)를 허용합니다. `OPENCODE_*`가 설정되지 않았다면 시작 시 `IRIS_*` 값이 복사됩니다. 새 스크립트에는 `IRIS_*` 사용을 권장합니다.


### Agents

IRIS 에는 내장 에이전트 2개가 있으며 `Tab` 키로 전환할 수 있습니다.
...
IRIS 설정에 대한 자세한 내용은 [upstream OpenCode 문서](https://opencode.ai/docs) 를 참고하세요. IRIS 전용 문서는 irislab.dev 에서 확인할 수 있습니다.

### 기여하기

IRIS 에 기여하고 싶다면, Pull Request 를 제출하기 전에 [contributing docs](./CONTRIBUTING.md) 를 읽어주세요.
...
- 100% 오픈 소스
- 특정 제공자에 묶여 있지 않습니다. [OpenCode Zen](https://opencode.ai/zen) 을 통해 제공하는 모델을 권장하지만, IRIS 는 Claude, OpenAI, Google 또는 로컬 모델과도 사용할 수 있습니다. 모델이 발전하면서 격차는 줄고 가격은 내려가므로 provider-agnostic 인 것이 중요합니다.
- 기본으로 제공되는 LSP 지원
- TUI 에 집중. IRIS 는 neovim 사용자와 [terminal.shop](https://terminal.shop) 제작자가 만들었으며, 터미널에서 가능한 것의 한계를 밀어붙입니다.
- 클라이언트/서버 아키텍처. 예를 들어 IRIS 를 내 컴퓨터에서 실행하면서 모바일 앱으로 원격 조작할 수 있습니다. 즉, TUI 프런트엔드는 가능한 여러 클라이언트 중 하나일 뿐입니다.
